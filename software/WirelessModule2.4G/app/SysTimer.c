...
ROM_FUNC void SysTimerInitialize()
{
    
}

ROM_FUNC static void timerExecute(SysTimer_t *timer)
{
    //not repeat
...
ROM_FUNC void SysTimerUnSet(SysTimer_t *timer)
{
    if(timer)
    {
        timer->alloced = 0;
...
ROM_FUNC SysTimer_t *SysTimerSet(SysTimerRun_t run, uint32_t span, uint8_t flag, void *arg)
{
    int i;
    uint8_t istate;
